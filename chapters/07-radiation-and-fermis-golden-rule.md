# Chapter 7 — Radiation and Fermi's Golden Rule

## TL;DR

- Fermi's golden rule — $W = (2\pi/\hbar)|\langle f|\hat{H}'|i\rangle|^2\rho(E_f)$ — is the most-used formula in quantum physics, and it was Dirac's, not Fermi's. It is the long-time limit of Chapter 6's sinc-squared lineshape when the final state is a continuum. The density-of-states factor is not an add-on; it is the physics.
...
Chapter 6 gave us the first-order transition probability for a sinusoidal perturbation $\hat{H}'(t) = \hat{V}\cos(\omega t)$ near resonance $\omega \approx \omega_{fi}$:
