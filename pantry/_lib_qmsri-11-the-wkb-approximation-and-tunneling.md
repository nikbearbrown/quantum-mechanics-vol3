...
- The chapter moves through The WKB ansatz, When the approximation is valid — and when it is not, Bound states: the Bohr-Sommerfeld condition, Tunneling: the Gamow factor, and related ideas.
...
![The WKB wave function: large amplitude where the classical particle is slow, rapid phase where it moves fast](../images/11-the-wkb-approximation-and-tunneling-fig-01.png)
*Figure 11.1 — Three panels showing ψ_WKB(x) in an arbitrary smooth*
...
![The Airy function is the exact solution near the turning point. Its asymptotic forms on each side pin down the WKB connection, including the π/4 Maslov phase](../images/11-the-wkb-approximation-and-tunneling-fig-02.png)
*Figure 11.2 — Diagram of the WKB connection at a single*
...
![The Bohr-Sommerfeld condition as a phase-space area quantization. Each allowed orbit encloses area (n+½)h. For the harmonic oscillator, this gives the exact energies](../images/11-the-wkb-approximation-and-tunneling-fig-03.png)
*Figure 11.3 — Phase-space diagram illustrating Bohr-Sommerfeld for the harmonic oscillator*
...
![WKB nails the exponent, misses the prefactor by an O(1) factor. The two curves are parallel below the barrier; the offset is the smooth function the leading WKB drops](../images/11-the-wkb-approximation-and-tunneling-fig-04.png)
*Figure 11.4 — T vs*
...
![The alpha tunnels from r = R to r = r_c through the Coulomb barrier. For ²³⁸U, the barrier is 8× wider than the nucleus. The Gamow factor γ ≈ 43 gives e^{−2γ} ≈ 10^{−38}](../images/11-the-wkb-approximation-and-tunneling-fig-05.png)
*Figure 11.5 — Coulomb barrier diagram for alpha decay *

![The Geiger-Nuttall law: log halflife is linear in 1/√E_α. Twenty-four decades of dynamic range on a single straight line, derived from the WKB Gamow factor](../images/11-the-wkb-approximation-and-tunneling-fig-06.png)
*Figure 11.6 — Geiger-Nuttall plot *
...
![The Gamow peak: fusion happens where the thermal tail and the tunneling probability overlap. The peak sits well above kT because tunneling is exponentially suppressed at low energy](../images/11-the-wkb-approximation-and-tunneling-fig-07.png)
*Figure 11.7 — Gamow peak diagram for stellar pp fusion *
